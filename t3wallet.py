...
def listen():
    ...
    while True:
        ...
        if b["type"] == "block":
            ...
            if b["miner"] == my_addr:
                balance += 100
            log.insert(tk.END, f"Блок {b['height']} • {b['miner'][:15]}...\n")
            bal_label.config(text=f"{balance:,} T3C")
            height_label.config(text=f"Сеть: {net_height}")
            log.see(tk.END)
...
tk.Label(root, text=my_addr, font=("Courier",12), fg="#00FF00", bg="black").pack()
...
log.insert(tk.END, "Подключено к сети T3Coin\n")
